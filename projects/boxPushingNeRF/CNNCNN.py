...
from src.objectEncoder2D import ObjectEncoder2D, ObjectEncoder2DSingleView, ResnetImageEncoder_1
...
class CNNCNN:
    ...
    def buildModel(self):
        ...
        if ('singleView' not in self.C) or (not self.C['singleView']):
            ...
            self.objectEncoder = ObjectEncoder2DSingleView(imageEncoder=CNN, k=self.C['k'], F=self.C['F'])
            self.latentStateAggreator = LatentStateAggregator_1(k=self.C['k'], k_imageDec=self.C['k_imageDec'])
            self.imageDecoder = ImageDecoder_SingleView(self.C['k_imageDec'], H=self.C['H'], W=self.C['W'])
...
if __name__ == "__main__":
    #cnncnn = CNNCNN(dataPath="dataGeneration/data/nM_1", expPath="exp/ConvAutoencoder-comp", k=64)
    #cnncnn.C['global'] = False
    
    cnncnn = CNNCNN(dataPath="dataGeneration/data/nM_1", expPath="exp/ConvAutoencoder-global")
