...
void CameraObs::getCameraObservations(torch::Tensor& I, torch::Tensor& M, torch::Tensor& KMat, torch::Tensor& KBounds) {
  ...
  for(uint cameraNumber = 0; cameraNumber < getNumberOfViews(); cameraNumber++) {
    ...
    for(rai::String objectFrameName : objectNames) {
      ...
      torch::Tensor mask_t = torch::from_blob(mask.p, {h, w}, torch::kByte).clone()*(unsigned char)255;
...
void CameraObs::addToHDF5File(HDF5File &file, uint index) {
  torch::Tensor I;
  torch::Tensor M;
  torch::Tensor KMat;
  torch::Tensor KBounds;
  getCameraObservations(I, M, KMat, KBounds);
  using SL = torch::indexing::Slice;
  KMat.index({SL(), 1, SL()}) *= -1.0f; // image coordinate system convention fix
  file.addToDataset<uint8_t>("I", I.unsqueeze(0), index);
  file.addToDataset<uint8_t>("M", M.unsqueeze(0), index);
  file.addToDataset<float>("K", KMat.unsqueeze(0), index);
  file.addToDataset<float>("KBounds", KBounds.unsqueeze(0), index);
}
