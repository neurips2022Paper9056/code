...
class Comp3DDyn:
    ...
    def trainEpochChunkedMasked(self, epoch):
        ...
        for b, bD in enumerate(self.trainLoader):
            ...
            M_R_smallObjects = torch.sum(M_R[:,1:], dim=1) > 0 # B, H, W "union of small masks"
            M_R = torch.sum(M_R, dim=1) > 0  # B, H, W "union" of masks

            M_R_enlarged = enlargeMasks(M_R.unsqueeze(1), 9).reshape(B, H*W) # B, H*W

            M_R_smallObjects = enlargeMasks(M_R_smallObjects.unsqueeze(1), 9).reshape(B, H*W).view(B, H*W) # B, H*W
            ...
            M = M[:,:,[0,2]] # door handle is not a separate object
            ...
            M_R_smallObjects_chunked = torch.split(M_R_smallObjects, NSplit, dim=1)
            M_R_chunked = torch.split(M_R, NSplit, dim=1)
            M_R_enlarged_chunked = torch.split(M_R_enlarged, NSplit, dim=1)
            batchLoss = 0.0

            for uvChunk, I_R_chunk, M_R_smallObjects_chunk, M_R_chunk, M_R_enlarged_chunk in zip(uvChunked, I_R_chunked, M_R_smallObjects_chunked, M_R_chunked, M_R_enlarged_chunked):
                N = uvChunk.shape[1]
                NP = self.C['NP']
                ...
                loss = (target - rgb).square().mean()
                
                # loss on small objects
                if self.C['lossOnSmallObjects'] > 0.0:
                    l = target[M_R_smallObjects_chunk] - rgb[M_R_smallObjects_chunk]
                    if l.numel():
                        loss += self.C['lossOnSmallObjects']*(l.square().mean())
    ...
    def __init__(self, **C):
        ...
        self.C['xLim'] = [-0.15-0.85/2, -0.15+0.85/2]
        self.C['yLim'] = [-0.03-0.2/2, -0.03+0.2/2]
        self.C['zLim'] = [0.2-0.4/2, 0.2+0.4/2]
        self.C['XW'] = 85
        self.C['XH'] = 20
        self.C['XD'] = 40
...
if __name__ == "__main__":
    comp3DDyn = Comp3DDyn(dataPath="data/1", expPath="exp/NeRF-RL-field", lossOnSmallObjects=1.0)
    comp3DDyn.setRandomSeed()
