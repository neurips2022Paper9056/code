...
class ImageDecoder_1(torch.nn.Module):
    ...
    def __init__(self, k_imageDec, H, W):
        ...
        nf = k_imageDec
        self.network = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(nf + 12, nf, (3, 4), 1, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf),
            torch.nn.ConvTranspose2d(nf, nf, (3, 4), 2, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf),
            torch.nn.ConvTranspose2d(nf, nf // 2, (6, 8), 2, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf // 2),
            torch.nn.ConvTranspose2d(nf // 2, nf // 2, (3, 4), 2, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf // 2),
            torch.nn.ConvTranspose2d(nf // 2, nf // 4, (3, 4), 2, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf // 4),
            torch.nn.ConvTranspose2d(nf // 4, 3, (3, 4), 2, 0)
        )
...
class LatentStateAggregator_Identity(torch.nn.Module):
    def __init__(self, k, k_imageDec):
        super().__init__()
...
class ImageDecoder_SingleView(torch.nn.Module):
    # adapted from https://github.com/YunzhuLi/comp_nerf_dy/blob/1da9eeb63115cb4219efffa1c0a99b76af6cb0c7/nerf_dy/models.py#L83
    def __init__(self, k_imageDec, H, W):
        super().__init__()
        self.H = H
        ...
        nf = k_imageDec
        self.network = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(nf, nf, (3, 4), 1, 0),
            torch.nn.ReLU(),
            torch.nn.InstanceNorm2d(nf),
            torch.nn.ConvTranspose2d(nf, nf, (3, 4), 2, 0),
            torch.nn.ReLU(),
    ...
    def forward(self, zAggr, KT):
        B, k_imageDec = zAggr.shape
        return self.network(zAggr.reshape(B, k_imageDec, 1, 1))[:,:,0:self.H,0:self.W]
