...
struct Imp_ObjectImpulseOnce : SimulationImp {
  Frame* obj;
  arr param;

  Imp_ObjectImpulseOnce(Frame* _obj, const arr& param) : obj(_obj), param(param) { CHECK(obj, "");  when = _beforePhysics;  }
  virtual void modConfiguration(Simulation& S, double tau);
};
...
void Simulation::addImp(Simulation::ImpType type, const StringA& frames, const arr& parameters) {
  ...
  } else if(type==_objectImpulseOnce) {
    CHECK_EQ(frames.N, 1, "");
    rai::Frame* obj = C.getFrame(frames(0));
    imps.append(make_shared<Imp_ObjectImpulseOnce>(obj, parameters));
...
void Imp_ObjectImpulseOnce::modConfiguration(Simulation& S, double tau) {
  std::shared_ptr<SimulationState> state = S.getState();

  state->frameVels(obj->ID, 0, {}) = param;
