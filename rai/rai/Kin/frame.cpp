...
rai::Frame::Frame(Configuration& _K, const Frame* copyFrame)
  ...
  if(copyFrame) {
    ...
    if(copyFrame->SDF) SDF = copyFrame->SDF;
    if(copyFrame->SDFz) SDFz = copyFrame->SDFz;
    if(copyFrame->ImSModule) ImSModule = copyFrame->ImSModule;
    if(copyFrame->ImBoundingBox) ImBoundingBox = copyFrame->ImBoundingBox;
    if(copyFrame->functionalObject) functionalObject = copyFrame->functionalObject;
  }
}
...
void rai::Frame::setSDF(std::string networkFileName, const arr& z, std::string device) {
 SDF = make_shared<torch::jit::script::Module>(torch::jit::load(networkFileName, device));
 if(!!z) SDFz = make_shared<torch::Tensor>(torch::from_blob(z.p, {z.d0}, torch::kDouble).to(torch::kFloat).to(device));
}



void rai::Frame::setFunctionalObject(const std::string& networkFileName, const arr& z) {
  ...
  functionalObject = make_shared<VectorFunctionWithInd> ([FNONetwork, z](arr& y, arr& J, const arr& x, int ind) {
...
arr rai::Frame::functionalObjectQuery(arr &J, const arr &x, int ind) {
  rai::Transformation pose = ensure_X();
  arr rot = pose.rot.getArr();
  arr x_rel = (~rot)*(x-conv_vec2arr(pose.pos));
  arr phi;
  (*functionalObject)(phi, J, x_rel, ind);
  if(!!J) J = J*~rot;
  return phi;
}
...
void rai::Frame::read(const Graph& ats) {
  ...
  if(ats["boundingBoxExtend"]) {
    ImBoundingBox = std::make_shared<ImS::BoundingBox>();
    ImBoundingBox->extractFromFrame(this);
  }
}
...
rai::Frame& rai::Frame::setRotationRad(double rad, double x, double y, double z) {
  ensure_X();
  X.rot.setRad(rad, x, y, z);
  X.rot.normalize();
  _state_updateAfterTouchingX();
  return *this;
}
...
rai::Shape::Shape(Frame& f, const Shape* copyShape)
  ...
  if(copyShape) {
    ...
    if(s.implicitFunctional) implicitFunctional = s.implicitFunctional;
    _type = s._type;
    size = s.size;
    cont = s.cont;
  } else {
...
shared_ptr<ScalarFunction> rai::Shape::functional(bool worldCoordinates) {
  ...
  if(implicitFunctional != nullptr) {
    ptr<ScalarFunction> FNO = make_shared<std::function<double(arr& g, arr& H, const arr& x)> > ([=](arr& g, arr& H, const arr& x)->double{
      arr rot = pose.rot.getArr();
      arr x_rel = (~rot)*(x-conv_vec2arr(pose.pos));
      double d = (*implicitFunctional)(g, H, x_rel);
      g = rot*g;
      H = rot*H*~rot;
      return d;
    });
