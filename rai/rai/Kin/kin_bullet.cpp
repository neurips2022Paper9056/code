...
struct BulletInterface_self {
  ...
  rai::Array<btTransform*> actorTransforms;
...
BulletInterface::BulletInterface(rai::Configuration& C, int verbose) : self(nullptr) {
  ...
  for(rai::Frame* a : links) {
    if(a->shape && a->shape->alpha() < 1.0) continue;
...
BulletInterface::~BulletInterface() {
  ...
  for(uint i = 0; i < self->actorTransforms.N; i++) {
    delete self->actorTransforms(i);
  }
}
...
void BulletInterface::pullDynamicStates(FrameL& frames, arr& frameVelocities) {
  ...
  for(rai::Frame* f : frames) {
    ...
    if(self->actorTypes(f->ID) == rai::BT_dynamic) {
      ...
      pose = pose*(*self->actorTransforms(f->ID)).inverse();
...
void BulletInterface::pushFullState(const FrameL& frames, const arr& frameVelocities) {
  for(rai::Frame* f : frames) {
    ...
    b->setWorldTransform(conv_raiTrans2btTrans(f->ensure_X())*(*self->actorTransforms(f->ID)));
...
btRigidBody* BulletInterface_self::addLink(rai::Frame* f, int verbose) {
  ...
  btCompoundShape* compoundShape = dynamic_cast<btCompoundShape*>(colShape);

  if(type == rai::BT_dynamic && compoundShape != nullptr) {
    btScalar masses[shapes.N];
    mass = 0.0;
    for(uint i = 0; i < shapes.N; i++) {
      double massOfPart = 0.0;
      rai::Inertia* inertiaOfPart = shapes(i)->frame.inertia;
      if(inertiaOfPart && inertiaOfPart->mass > 0.0) {
        massOfPart = inertiaOfPart->mass;
      }
      masses[i] = massOfPart;
      mass += massOfPart;
    }
    compoundShape->calculatePrincipalAxisTransform(masses, principal, localInertia);
    btCompoundShape* transformedCompoundShape = new btCompoundShape();
    collisionShapes.push_back(transformedCompoundShape);
    //    transformedCompoundShape->addChildShape(principal.inverse(), colShape);
    for(int i = 0; i < compoundShape->getNumChildShapes(); i++) {
      transformedCompoundShape->addChildShape(principal.inverse()*compoundShape->getChildTransform(i), compoundShape->getChildShape(i));
    }

    btDefaultMotionState* motionState = new btDefaultMotionState(pose*principal);
    body = new btRigidBody(btRigidBody::btRigidBodyConstructionInfo(mass, motionState, transformedCompoundShape, localInertia));
    ...
    body = new btRigidBody(btRigidBody::btRigidBodyConstructionInfo(mass, motionState, colShape, localInertia));
  }


  double fric=1.;
  double rollingFriction = 0.001;
  double spinningFriction = 0.001;
  if(shapes.N==1 && f == &shapes.scalar()->frame) {
    //try to read friction from attributes
    shapes.scalar()->frame.ats.get<double>(fric, "friction");
    shapes.scalar()->frame.ats.get<double>(spinningFriction, "spinningFriction");
    shapes.scalar()->frame.ats.get<double>(rollingFriction, "rollingFriction");
  }
  body->setFriction(fric);
  if(rollingFriction > 0.0) body->setRollingFriction(rollingFriction); //.001
  if(spinningFriction > 0.0) body->setSpinningFriction(spinningFriction);
...
//  body->setContactStiffnessAndDamping(1e7, 3e4);
  ...
  actors(f->ID) = body;
  while(actorTransforms.N<=f->ID) actorTransforms.append(0);
  actorTransforms(f->ID) = new btTransform(principal);
  return body;
}
...
rai::Array<btTransform*>::memMove=true;
