...
struct Frame : NonCopyable {
  ...
  ptr<torch::jit::Module> SDF;
  ptr<at::Tensor> SDFz;

  ptr<ImS::Module> ImSModule;
  ptr<ImS::BoundingBox> ImBoundingBox;

  ptr<VectorFunctionWithInd> functionalObject = make_shared<VectorFunctionWithInd> ([](arr& y, arr& J, const arr& x, int ind) {
                                                                 y = x;
                                                                 if(!!J) J = eye(3);
                                                          });
 ...
 public:
  ...
  void setSDF(std::string networkFileName, const arr& z = NoArr, std::string device="cuda");

  void setFunctionalObject(const std::string& networkFileName, const arr& z);
  arr functionalObjectQuery(arr& J, const arr& x, int ind);
  ...
  Frame& setRotationRad(double rad, double x, double y, double z);
...
struct Shape : NonCopyable, GLDrawer {
  ...
  ptr<ScalarFunction> implicitFunctional = nullptr;
