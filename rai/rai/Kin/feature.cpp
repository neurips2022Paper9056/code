...
StringA Feature::getFrameNames(const rai::Configuration& C) {
  StringA names;
  if(frameIDs.nd == 1){
    names.resize(frameIDs.N);
    for(uint k=0;k<frameIDs.N;k++) names(k) = C.frames.elem(frameIDs(k))->name;
  }else{
    names.resize(1);
    rai::String name;
    name << "#" << STRING(frameIDs.N);
    names(0) = name;
  }
  return names;
}
