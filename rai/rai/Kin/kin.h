...
struct Configuration : GLDrawer {
  ...
  FrameL getJointsSlice(const FrameL& slice, bool activesOnly) const;
  ...
  void setJointState(const arr& _q, const uintA& F, bool activesOnly=true){ setJointState(_q, getFrames(F), activesOnly); } ///< same as setJointState() with getFrames()
