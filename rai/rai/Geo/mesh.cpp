...
void rai::Mesh::setImplicitSurface(double *data, double xLo, double xHi, double yLo, double yHi, double zLo, double zHi, uint res) {
  MarchingCubes mc(res, res, res);
  mc.init_all();
  mc.set_ext_data(data);
  mc.run();
  mc.clean_temps();
  ...
  T.resize(mc.ntrigs(), 3);
  for(uint i=0; i<V.d0; i++) {
    V(i, 0)=xLo+mc.vert(i)->x*(xHi-xLo)/(res-1);
    V(i, 1)=yLo+mc.vert(i)->y*(yHi-yLo)/(res-1);
    V(i, 2)=zLo+mc.vert(i)->z*(zHi-zLo)/(res-1);
  }
  for(uint i=0; i<T.d0; i++) {
    T(i, 0)=mc.trig(i)->v1;
    T(i, 1)=mc.trig(i)->v2;
...
void rai::Mesh::setImplicitSurface(double *data, double xLo, double xHi, double yLo, double yHi, double zLo, double zHi, uint resX, uint resY, uint resZ) {
  MarchingCubes mc(resX, resY, resZ);
  mc.init_all();
  mc.set_ext_data(data);
  mc.run();
  mc.clean_temps();
  ...
  T.resize(mc.ntrigs(), 3);
  for(uint i=0; i<V.d0; i++) {
    V(i, 0)=xLo+mc.vert(i)->x*(xHi-xLo)/(resX-1);
    V(i, 1)=yLo+mc.vert(i)->y*(yHi-yLo)/(resY-1);
    V(i, 2)=zLo+mc.vert(i)->z*(zHi-zLo)/(resZ-1);
  }
  for(uint i=0; i<T.d0; i++) {
    T(i, 0)=mc.trig(i)->v1;
    T(i, 1)=mc.trig(i)->v2;
