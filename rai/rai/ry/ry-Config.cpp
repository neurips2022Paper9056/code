...
//-- setup/edit the configuration
  ...
  .def("setFNO",
       [](shared_ptr<rai::Configuration>& self, const std::string& frameName, const std::string& networkFileName, const arr& z) {
         rai::Frame* frame = self->getFrame(frameName.c_str());
         frame->setFunctionalObject(networkFileName.c_str(), z);
//         if(networkFileName.empty()){
//           frame->functionalObject = make_shared<VectorFunctionWithInd> ([](arr& y, arr& J, const arr& x, int ind) {
//             y = x;
//             if(!!J) J = eye(3);
//           });
//         }else{
//           shared_ptr<torch::jit::script::Module> FNONetwork = make_shared<torch::jit::script::Module>(torch::jit::load(networkFileName, std::string("cpu")));
//           frame->functionalObject = make_shared<VectorFunctionWithInd> ([FNONetwork, z](arr& y, arr& J, const arr& x, int ind) {
//             torch::Tensor xInput = torch::from_blob(x.p, {1, x.d0}, torch::kDouble).to(torch::kFloat);
//             torch::Tensor zInput = torch::from_blob(z.p, {1, z.d0}, torch::kDouble).to(torch::kFloat);
//             torch::IValue requireJacobian = !!J?true:false;
//             torch::IValue featureIndex = ind;

//             std::vector<torch::jit::IValue> inputs = {xInput, zInput, featureIndex, requireJacobian};
//             auto output = FNONetwork->forward(inputs).toTuple()->elements();
//             torch::Tensor yTensor = output[0].toTensor().to(torch::kDouble);
//             arr yd(yTensor.data_ptr<double>(), sizeof(torch::kDouble)*yTensor.numel(), true);
//             y = yd;

//             if(!!J){
//                torch::Tensor JTensor = output[1].toTensor().to(torch::kDouble);
//                arr Jd(JTensor.data_ptr<double>(), sizeof(torch::kDouble)*JTensor.numel(), true);
//                J = Jd.reshape(y.d0,x.d0);
//              }
//           });
//         }
       } )
...
To get really precise distances and penetrations use the FS.distance feature with the two frame names",
  ...
  .def("computeDistance", [](shared_ptr<rai::Configuration>& self, const ry::I_StringA& frameNames, const std::vector<double>& x) {
  arr dist(frameNames.size());
  for(uint i=0; i<dist.N; i++) {
    auto func = self->getFrame(frameNames[i].c_str())->shape->functional();
    dist(i) = (*func)(NoArr, NoArr, arr(x, true));
  }
  return dist.min();
}
)
...
//===========================================================================
  ...
  .def("addSensorFromFrame", [](ry::RyCameraView& self, const char* frameAttached) {
  ...
  .def("computePointCloudOnMask", [](ry::RyCameraView& self, const char* frameName) {
    byteA img;
    floatA depth;
    self.cam->computeImageAndDepth(img, depth);
    byteA mask;
    self.cam->computeMask(mask, frameName);
    arr pc = self.cam->computePointCloudOnMask(depth, mask);
    return pybind11::array(pc.dim(), pc.p);
  }, "",
  pybind11::arg("frameName"))

  .def("computeMask", [](ry::RyCameraView& self, const char* frameName) {
    byteA mask;
    self.cam->computeMask(mask, frameName);
    return mask;
//    return pybind11::array(mask.dim(), mask.p);
  }, "",
  pybind11::arg("frameName"))


  .def("getCameraRays", [](ry::RyCameraView& self) {

    uint width = self.cam->currentSensor->width;
    uint height = self.cam->currentSensor->height;

    arr pos = conv_vec2arr(self.cam->currentSensor->pose().pos);
    arr rays(height,width,3);

    for(uint i=0; i<height; i++){
      for(uint j=0; j<width; j++){
        arr ray{(double)j,(double)(height-i-1),1.};
        self.cam->currentSensor->cam.unproject_fromPixelsAndTrueDepth(ray, width, height);
        ray -= pos;
//        ray /= length(ray);
        for(uint k=0; k<3; k++) rays(i,j,k) = ray(k);
      }
    }

    return pybind11::array(rays.dim(), rays.p);
  }, "")
